...
public interface CompRepository extends JpaRepository<CompBean, Integer>{
	...
	@Query(value="SELECT * FROM competitions X LEFT JOIN games Y ON Y.game_id = X.game_id WHERE Y.game_id = ?1 and Y.deadline < ?2",nativeQuery=true)
	  List<CompBean> findByGame_idLike(Integer game_id, LocalDateTime now); //game_idの参照の仕方がわからない　JoinColumn
	...
	@Query("SELECT X FROM CompBean X WHERE X.end_date >= ?1 AND X.start_date <= ?2 AND X.start_date > ?3 ORDER BY X.comp_id")
	 List<CompBean> findOnlyStart(LocalDateTime now,LocalDateTime check,LocalDateTime check2);

$(function(){
    $("#comment_form").on("submit", function(e){
      e.preventDefault(); 
      var comp_id = document.getElementById("comp_id_for_comment").value;
      var ngWordList = document.getElementById("ngWordList").value;
      var change = ngWordList.replace("[", "").replace("]","");
      const arr = change.split(', ');
      const comment = $("#comment").val();
      var flg = 0;
      arr.forEach(elm => {
        if(comment.includes(elm)){
          flg = 1;
        }
        
      })
      if(flg == 1){
        alert("NGワードが含まれているためコメントできません");
      }
      else{
        $.ajax({
